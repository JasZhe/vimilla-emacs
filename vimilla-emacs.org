...
** prevent annoying scroll wheel behaviour on trackpads
#+begin_src elisp
...
** pseudo git gutter from emacs-solo
can use this for now or as inspiration
maybe we can leverage vc-mode itself though similar to diff-hl (but a discount diff-hl) so it's more portable across vc's
...
** Fun
#+begin_src elisp
(package-vc-install '(snow :url "https://github.com/alphapapa/snow.el"))
#+end_src
